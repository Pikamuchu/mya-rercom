...
/*************** Validación version DEMO **************************************/

  tmp = senal->longitud / senal->frec_muestreo;
  
  if (tmp > MAX_DURACION_SENAL){
    printf("\n\nLimitacion version DEMO: El fichero %s tiene una duración de %f segundos.",fichero_in,tmp);
    printf("\n\nLa version DEMO está limitada a %d segundos.\n",MAX_DURACION_SENAL);    
    exit(-1);
  }
